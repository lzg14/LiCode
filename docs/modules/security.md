...
- `rm`, `del` — 删除（必须用 licode 的安全删除）
- `sudo`, `su` — 提权
- `chmod`, `chown` — 权限修改
- `python`, `python3` — 需通过 licode 工具调用
- `exec`, `eval` — 代码执行
...
**即使在允许目录下，关键文件的修改也需要用户确认**：

| 文件类型 | 示例 | 确认级别 |
|----------|------|----------|
| 全局配置 | `package.json`, `tsconfig.json`, `.pai/config` | 🔴 强制确认 |
| 环境配置 | `.env`, `.env.*`, `config.*` | 🔴 强制确认 |
| 依赖文件 | `Cargo.toml`, `go.mod`, `requirements.txt` | 🟡 提示确认 |
| 入口文件 | `index.ts`, `main.py`, `main.go` | 🟡 提示确认 |
| 敏感文件 | `*.key`, `*.pem`, `*.secret` | 🔴 强制确认 |

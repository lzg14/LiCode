...
**核心原则**：
- 工具是 opague 类型，输入输出 codec 自包含
- Schema 转换不能依赖服务
- 工具依赖在构造时获取并捕获到 execute 中

### 6.5 工具调用上下文（Tool.Context）
...
## 7. 安全层设计（Security Layer）

### 7.1 设计背景

Claude Code、opencode、DevEco Code 等项目的代码分布分析：
...
| 类别 | 操作 | 记录内容 |
|------|------|----------|
| **命令执行** | 所有 shell 命令 | 命令、参数、退出码 |
| **文件操作** | Read/Write/Edit/Delete | 文件路径、操作类型 |
| **Git 操作** | commit/push/pull | 仓库、操作 |
| **安全事件** | 阻止/警告 | 事件类型、详情 |

### 8.3 费用追踪（参考 DevEco Code）

| 统计项 | 说明 |
|--------|------|
| `tokens_used` | 估算的 token 消耗 |
| `cost_estimate` | 费用估算 |
| `commands_executed` | 本会话命令数 |
| `files_modified` | 修改的文件数 |

---

## 9. 配置层设计（Config Layer）
...
llm:
  ...
  api_key_env: "ANTHROPIC_API_KEY"   # 不硬编码
...
### 9.3 Provider 切换（参考 DevEco Code）
...
### 10.1 Effort Level 路由（参考 PAI Algorithm v6.3.0）
...
### 7.7.2 多话题隔离机制（Multi-Topic Isolation）

**痛点：** 用户在一个 session 中可能突然问另一个项目的问题，导致上下文混杂，影响回答质量。

**核心思想：** 自动检测话题切换，提示用户隔离上下文，避免无关信息干扰。
...
- [opencode](https://github.com/s上一层/opencode) - 核心架构参考，被多个项目基于此开发
- [DevEco Code](https://github.com/CarSmallGuo/deveco-code) - 华为 HarmonyOS AI 助手，基于 opencode，Provider 切换、费用追踪参考

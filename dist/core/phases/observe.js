...
function estimateEffortLevel(input) {
    if (input.length < 50)
        return 1;
    if (input.includes('?'))
        return 2;
    if (input.includes('帮我') || input.includes('帮我搞'))
        return 3;
    if (input.includes('重新设计') || input.includes('架构'))
        return 5;
    return 4;
}
